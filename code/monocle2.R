...
# 接下来需要根据临床属性，对这个 monocleObj 进行找寻差异基因 
###
...
length(monocleObj_ordering_genes)
...
dir.create('../results/')
plot_cell_trajectory(monocleObj, color_by = 'Grade') +
  scale_color_manual(values = c('Normal'='#bdc1c8', 'FibCS'='#67814f', 'DN'='#f2ae2e', 'T1'='#eca9a4', 'T2'='#e65251', 'T3-4'='#963e38', 'Mixed'='#7a41c4'))
...
plot_cell_trajectory(monocleObj, color_by = 'op') + 
...
plot_cell_trajectory(monocleObj, color_by = 'Tregs') +
  scale_color_gradient(low = 'grey90', high = 'red2')
ggsave('../results/disp.Tregs.pdf', units = 'cm', width = 9, height = 10)


plot_cell_trajectory(monocleObj, color_by = 'AbsoluteScore') + 
  scale_color_gradient(low = 'grey90', high = 'red2')
